...
class EnvSet(gym.Env):
    def __init__(self, env_fns):
        ...
        self.action_splits = np.concatenate([e.action_space.shape for e in self.envs])
        ...
        obs_splits = np.concatenate([e.observation_space.shape for e in self.envs])
    ...
    def step(self, action):
        ...
        a = torch.split(action, self.action_splits)
        ...
        reward = np.concatenate([env_state[i][1] for i in range(len(env_state))]).sum()
